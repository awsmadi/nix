...
struct TarAdapterImpl : merkle::TarAdapter
{
    ...
    /**
     * Does `path` have any entries underneath it?
     *
     * `CanonPath`'s ordering puts a directory directly before its
     * children, so it suffices to look at the next entry.
     */
    bool hasChildren(const CanonPath & path)
    {
        auto it = entries.upper_bound(path);
        return it != entries.end() && it->first.isWithin(path);
    }

    /**
     * Insert an entry, applying tarball unpacking overwrite semantics.
     *
     ...
     * is an error.
     */
    void insertEntry(const CanonPath & path, Entry && entry)
    {
    ...
    {
        insertEntry(path, DirectoryMarker{});
    }
    ...
    void createRegularFile(const CanonPath & path, bool isExecutable, fun<void(Sink &)> callback) override
    {
        ...
        insertEntry(path, PendingFile{sink, mode});
    }

    void createSymlink(const CanonPath & path, const std::string & target) override
    {
        auto entry = store.makeSymlink(target);
        insertEntry(path, CompletedFile{entry.hash, entry.mode});
    }

    void createHardlink(const CanonPath & path, const CanonPath & target) override
    {
        insertEntry(path, PendingHardlink{target});
    }
    ...
    merkle::TreeEntry flush() override
    {
        ...
        while (!pendingHardlinks.empty()) {
            ...
            for (auto * pathPtr : pendingHardlinks) {
                ...
                if (targetIt == entries.end()) {
                    throw Error("cannot create hard link '%s': target '%s' does not exist", *pathPtr, link.target);
                }
                std::visit(
                    overloaded{
                        ...
                        [&](DirectoryMarker &) {
                            throw Error("cannot create hard link '%s' to a directory '%s'", *pathPtr, link.target);
                        },
                    },
                    targetIt->second);
            }
            if (stillPending.size() == pendingHardlinks.size()) {
                throw Error("cannot create hard link '%s': hard link cycle detected", **stillPending.begin());
            }
        ...
        if (entries.size() == 1) {
            ...
            assert(std::holds_alternative<DirectoryMarker>(entry));
